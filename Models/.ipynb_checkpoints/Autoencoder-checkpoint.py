...
class AutoencoderCompressingChannels(nn.Module):
    def __init__(self, stride, kernel_n, n_feature_maps = 3):
        super().__init__()
        n1 = kernel_n
        s = stride
        self.encoder = nn.Sequential(
            nn.Conv2d(1, n_feature_maps, kernel_size = (64, 2*n1 + 1), padding = (0, n1), stride = (1, s)), 
            nn.ELU(inplace = True),
        )
...
#         self.unpool  = nn.MaxUnpool2d(kernel_size = (1, 2), stride = (1, 2))
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(n_feature_maps, 1, kernel_size = (64, 2*n1 + 1), padding = (0, n1), stride = (1, s), output_padding = (0, s - 1))
        )
