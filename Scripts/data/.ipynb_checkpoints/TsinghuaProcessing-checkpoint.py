...
def splitDataset(dataset:str, outputPath:str, time:int, inputPath:str = 'D://Datasets//Tsinghua'):
    ...
    f = partial(_splitDataset, inputPath = datasetPath, outputPath = outputPath, n_segments = n_segments)
    with Pool(processes = 4) as pool:
        pool.map(f, sublists)
    
def _splitDataset(classes: List[int], inputPath, outputPath, n_segments:int):
    for c in classes:
